...
const GuessNumberPage = () => {
    ...
    const [restartButton, setRestartButton] = useState("invisible");
    ...
    const handleGuess= () => {
        ...
        if (Number(guess) > 100 || Number(guess) < 1) {
            ...
            setDetailsColor("text-red-600")
        } else if (Number(guess) < secret) {
            setResult(`The secret number is higher than ${guess}`)
            setAttempts(prev => prev + 1)
            setDetailsColor("text-green-600");
        } else if (Number(guess) === secret) {
            setResult(`You Win the secret number is ${guess}`)
            setRestartButton("visible")
            setDetailsColor("text-blue-600");
        }
        setGuess("")
    }
    ...
    return (
        <>
            ...
            <div className="text-center py-6">
                <h1 className={`text-center ${detailsColor}`} py-4>{result}</h1>
                <h1 className="py-4">Attempts: {attempts}</h1>
            </div>
            <div className="text-center py-12">
                <button
                    onClick={handleRestart}
                    className={`bg-cyan-900 border-4 text-cyan-400 w-16 rounded-4xl hover:font-bold ${restartButton}`}
                >Restart</button>
            </div>
        </>
    )
